<x-app-layout>
    ...
    {{ $question->ans }}
        <form x-data="{ ans: '{{ $question->ans }}' }" action="{{ route('admin.distance.question.update',$question->id) }}" method="post">
            @csrf
            <div class="mt-1 flex flex-col p-1">

                <textarea name="content" class="rounded"> {{ $question->content }}</textarea>
